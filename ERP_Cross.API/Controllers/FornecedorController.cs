...
public class FornecedorController(FornecedorService service) : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<FornecedorView>>> GetAll([FromQuery] string? q)
        => Ok(await _service.GetAllAsync(q));

    [HttpGet("{id:int}")]
    public async Task<ActionResult<FornecedorView>> GetById(int id)
    {
    ...
    [HttpPost]
    public async Task<ActionResult<FornecedorView>> Create(CreateFornecedorDto dto)
    {
