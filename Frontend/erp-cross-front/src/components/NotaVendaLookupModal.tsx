...
interface Props {
  onSelect: (nota: NotaVendaView) => void;
  onClose: () => void;
  zBase?: number;
}

export default function NotaVendaLookupModal({ onSelect, onClose, zBase = 1000 }: Props) {
  const [all, setAll] = useState<NotaVendaView[]>([]);
  const [search, setSearch] = useState('');
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    NotaVendaService.getAll()
    ...
    <div className="modal-overlay" style={{ zIndex: zBase }} onClick={onClose}>
      <div className="modal modal-lookup" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <h2>Selecionar Nota de Venda</h2>
        ...
        <div className="modal-body lookup-modal-body">
          ...
          {loading ? (
            ...
            <div className="table-empty">Nenhuma nota de venda encontrada.</div>
          ) : (
