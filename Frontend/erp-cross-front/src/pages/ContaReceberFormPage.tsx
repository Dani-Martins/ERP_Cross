...
import type { ContaReceberCreate, ClienteView, FormaPagamentoView, NotaVendaView } from '../types/entities';
...
import NotaVendaLookupModal from '../components/NotaVendaLookupModal';
...
export default function ContaReceberFormPage() {
  ...
  const [selectedNotaVenda, setSelectedNotaVenda] = useState<NotaVendaView | null>(null);
  ...
  const [showNotaVendaLookup, setShowNotaVendaLookup] = useState(false);
  ...
  function handleNotaVendaSelected(nota: NotaVendaView) {
    setSelectedNotaVenda(nota);
    setShowNotaVendaLookup(false);
  }

  function handleLimparNota() {
    setSelectedNotaVenda(null);
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      if (isEdit) {
        if (!form.clienteId) { setError('Selecione o Cliente.'); setSaving(false); return; }
        ...
        if (!selectedNotaVenda) { setError('Selecione a Nota de Venda.'); setSaving(false); return; }
        if (!form.dataVencimento) { setError('Data de vencimento é obrigatória.'); setSaving(false); return; }
        await ContaReceberService.create({
          numeroNota: selectedNotaVenda.numeroNota,
          modelo: selectedNotaVenda.modelo,
          serie: selectedNotaVenda.serie,
          clienteId: selectedNotaVenda.clienteId,
          numParcela: 1,
          valorParcela: selectedNotaVenda.totalPagar,
          dataEmissao: selectedNotaVenda.dataEmissao,
  ...
  return (
    <>
                  ...
                  <div className="form-group">
                    ...
                    <div className="lookup-field">
                      <input type="text" readOnly className="lookup-input"
                        value={selectedNotaVenda ? `${selectedNotaVenda.numeroNota} - ${selectedNotaVenda.nomeCliente ?? ''}` : ''}
                        placeholder="Selecione uma nota de venda..." />
                      <button type="button" className="btn-lookup"
                        onClick={() => setShowNotaVendaLookup(true)}>
                        <Search size={15} />
                      </button>
                    </div>
                  </div>
                  {selectedNotaVenda && (
                    <>
                      <table className="data-table" style={{ marginBottom: '1rem' }}>
                        ...
                        <tbody>
                          <tr>
                            <td>{selectedNotaVenda.totalPagar.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
                            <td>{selectedNotaVenda.nomeCondicaoPagamento ?? '—'}</td>
                          </tr>
                        </tbody>
                      </table>
                      <div className="form-group">
      ...
      {/* Lookup — Nota de Venda */}
      {showNotaVendaLookup && (
        <NotaVendaLookupModal
          onSelect={handleNotaVendaSelected}
          onClose={() => setShowNotaVendaLookup(false)}
          zBase={1000}
        />
      )}
