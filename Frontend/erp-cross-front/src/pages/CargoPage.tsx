...
import { Plus, Pencil, Trash2, Search, X, Briefcase, Eye } from 'lucide-react';
      ...
      <div className="table-card">
        {loading ? (
          ...
          <div className="table-empty">
            {search ? 'Nenhum cargo encontrado para a busca.' : 'Nenhum cargo cadastrado.'}
          </div>
        ) : (
          <table className="data-table">
            ...
            <tbody>
              {filtered.map((cargo) => (
                <tr key={cargo.id}>
                  <td className="col-id">{cargo.id}</td>
                  ...
                  <td>{cargo.exigeCnh ? 'Sim' : 'Não'}</td>
                  <td>
                    <span className={`status-badge ${cargo.ativo ? 'status-active' : 'status-inactive'}`}>
                  ...
                  <td className="col-actions">
                    <button
                      ...
                      title="Visualizar"
                      onClick={() => navigate(`/cargos/visualizar/${cargo.id}`)}
                    >
                      <Eye size={15} />
                    </button>
                    <button
