...
export default function FuncionariosPage() {
  ...
  const filtered = funcionarios.filter((f) => {
    const matchSearch =
      ...
      (f.nomeCargo ?? '').toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="page-container">
      <div className="page-header">
        ...
        <div className="page-actions">
          ...
          <button className="btn-primary" onClick={() => navigate('/funcionarios/novo')}>
      ...
      <div className="table-card">
        {loading ? (
          ...
          <div className="table-empty">
            {search ? 'Nenhum funcionário encontrado para a busca.' : 'Nenhum funcionário cadastrado.'}
          </div>
        ) : (
            ...
            <tbody>
              {filtered.map((f) => (
                <tr key={f.id}>
                  ...
                  <td><span className="tag">{maskCpf(f.cpfCnpj)}</span></td>
                  <td>{f.nomeCargo ?? '—'}</td>
                  <td>{f.nomeCidade ?? '—'}</td>
                  <td>
                    {f.salario != null
                  ...
                  <td className="col-actions">
                    <button
                      ...
                      title="Visualizar"
                      onClick={() => navigate(`/funcionarios/visualizar/${f.id}`)}
                    >
                      <Eye size={15} />
                    </button>
                    <button
        ...
        <div className="modal-overlay" onClick={() => setDeleteId(null)}>
          <div className="modal modal-sm" onClick={(ev) => ev.stopPropagation()}>
            ...
            <div className="modal-body">
              <p>Tem certeza que deseja excluir este funcionário? Esta ação só pode ser desfeita por um Administrador.</p>
            </div>
